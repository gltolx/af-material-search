#!/usr/bin/env bash
# 一条命令把 /broll 技能装进本机的 Claude / Codex 客户端 + 装环境 + 自检。
...
SRC="$ROOT/skills/broll/SKILL.md"                       # 含 {{BROLL_HOME}} 占位的技能模板
[ -f "$SRC" ] || { echo "  [失败] 缺技能模板 $SRC"; exit 1; }

echo "== 安装 /broll 技能(BROLL_HOME=$ROOT)=="
...
if [ "$PF" -eq 0 ]; then
  ...
  echo "⚠️  技能已装($n 个客户端);preflight 有红灯(多半是 CLT/登录态)→ 按上面提示修后重跑。"
fi
exit "$PF"
