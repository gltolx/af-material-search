...
main(){
  ...
  git rev-parse --is-inside-work-tree >/dev/null 2>&1 || { echo "  [跳过] 非 git 仓(tarball 部署),不自动更新"; return 0; }
  git remote get-url origin >/dev/null 2>&1 || { echo "  [跳过] 无 origin 远端,不自动更新"; return 0; }
  ...
  if ! GIT_SSH_COMMAND="ssh -o ConnectTimeout=10 -o BatchMode=yes" git fetch -q origin 2>/dev/null; then
    echo "  [跳过] git fetch 失败(离线/无权限?),用本地代码继续"; return 0
  fi
  ...
  [ -z "$REMOTE" ] && { echo "  [跳过] 远端无 $BR 分支"; return 0; }
  [ "$LOCAL" = "$REMOTE" ] && { echo "  [OK] 代码已是最新($BR @ ${LOCAL:0:7})"; return 0; }
