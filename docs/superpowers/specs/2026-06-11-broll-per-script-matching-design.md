...
- **配额（按口播稿总数 N 分档，稿越多每稿越少）**：**N≥10 → 2~5 条/稿**（常态）；**5≤N≤9 → 4~6 条/稿**；**N<5 → 5~7 条/稿**（少稿多配）。边界：N=10 归 ≥10 档（2~5）。同档内长稿（正文>500字）取偏上限。**不硬凑**——优质匹配明显不足（/<2 条）记"⚠ 欠匹配"，提示补搜。（2026-06-12 按用户修订：原"统一 2~5 + 长稿更多"改为按稿数分档。）
- **独占**：每个素材只挂最适配的一条稿。
...
## 验收
- 给一批带/不带标题与人设号的稿，跑完流水线后：`filtered.html` 中匹配素材**已自动勾选**，卡片显示人设/稿名；每稿条数按稿数分档（N≥10→2~5、5~9→4~6、N<5→5~7）、独占不重复；明显人物主体素材落 drop/review；>20 分钟素材落 drop。
